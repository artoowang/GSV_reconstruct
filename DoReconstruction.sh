...
function usage () {
	echo >&2 "Usage: $0 [-g|--GPS-prematching] <landmark_name> <img_set_name>"
	exit
}
...
if ! options=$(getopt -o g -l GPS-prematching -- "$@"); then
...
while [ $# -gt 0 ]; do
	case $1 in
		-g|--GPS-prematching) GPS_prematching=1; echo >&2 "Enable GPS prematching" ;;
		(--) shift; break;;
...
# eval is required from getopt. See man page.
eval "LID=$1"
LID_STR="$LID"
DIR="data/$LID_STR"
...
if [ $GPS_prematching = "1" ]; then
	export DO_GPS_PREMATCHING=1	# Do prematching with GPS coordinates
fi
